...
from physics.PhysicalConstraintModule import PhysicalConstraintModule
import datasets
...
def log_validation(
    ...
    try:
        ...
        pipeline = merge_lora(
            pipeline, None, 1, accelerator.device, state_dict=accelerator.unwrap_model(network).state_dict(),
            transformer_only=True
        )

        if args.enable_xformers_memory_efficient_attention \
                and config['transformer_additional_kwargs'].get('transformer_type',
                ...
                if args.train_mode != "normal":
                    with torch.autocast("cuda", dtype=weight_dtype):
                        if vae.cache_mag_vae:
                            video_length = int((
                                                           args.video_sample_n_frames - 1) // vae.mini_batch_encoder * vae.mini_batch_encoder) + 1 if args.video_sample_n_frames != 1 else 1
                        else:
                            video_length = int(
                                args.video_sample_n_frames // vae.mini_batch_encoder * vae.mini_batch_encoder) if args.video_sample_n_frames != 1 else 1
                        input_video, input_video_mask, clip_image = get_image_to_video_latent(None, None,
                        ...
                        save_videos_grid(sample,
                                         os.path.join(args.output_dir, f"sample/sample-{global_step}-image-{i}.gif"))
                else:
                    with torch.autocast("cuda", dtype=weight_dtype):
                        sample = pipeline(
                            args.validation_prompts[i],
...
def main():
    ...
    physics_module = PhysicalConstraintModule()
        ...
        if config['text_encoder_kwargs'].get('enable_multi_text_encoder', False):
            ...
            if config['text_encoder_kwargs'].get('replace_t5_to_llm', False):
                text_encoder_2 = Qwen2VLForConditionalGeneration.from_pretrained(
                    os.path.join(args.pretrained_model_name_or_path, "text_encoder_2"), revision=args.revision,
                    variant=args.variant,
                    torch_dtype=weight_dtype,
                )
            else:
            ...
            if config['text_encoder_kwargs'].get('replace_t5_to_llm', False):
                text_encoder = Qwen2VLForConditionalGeneration.from_pretrained(
                    os.path.join(args.pretrained_model_name_or_path, "text_encoder"), revision=args.revision,
                    variant=args.variant,
                    torch_dtype=weight_dtype,
                )
            else:
    ...
    network.apply_to(text_encoder, transformer3d, args.train_text_encoder and not args.training_with_video_token_length,
                     True)
    ...
    if args.enable_bucket:
        ...
        def get_length_to_frame_num(token_length):
            ...
            if vae.cache_mag_vae:
                ...
                length_to_frame_num = {
                    sample_size: min(token_length / sample_size / sample_size,
                                     args.video_sample_n_frames) // sample_n_frames_bucket_interval * sample_n_frames_bucket_interval
                    for sample_size in sample_sizes
                }
        ...
        def collate_fn(examples):
            ...
            for example in examples:
                ...
                if args.train_mode != "normal":
                    ...
                    mask_pixel_values = new_examples["pixel_values"][-1] * (1 - mask) + torch.ones_like(
            ...
            if args.train_mode != "normal":
                new_examples["mask_pixel_values"] = torch.stack(
                    [example[:batch_video_length] for example in new_examples["mask_pixel_values"]])
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            state_dict = load_file(
                #os.path.join(os.path.join(args.output_dir, path), "lora_diffusion_pytorch_model.safetensors"))
                                os.path.join(args.output_dir, "checkpoint-"+str(global_step)+".safetensors"))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            #
            pixel_values = batch['pixel_values'].cpu().mean(dim=2, keepdim=True)
            pixel_values = rearrange(pixel_values, 'b t c (h_split h) (w_split w) -> b t (c h_split w_split) h w', h_split=2, w_split=2)
            # print(pixel_values.shape)
            # pixel_values = rearrange(batch['pixel_values'].cpu(), 'b t (2 h) (2 w) -> b t 4 h w')
            ...
            with accelerator.accumulate(transformer3d):
                ...
                if args.random_frame_crop:
                    ...
                    if vae.cache_mag_vae:
                        ...
                        select_frames = [_tmp for _tmp in list(range(sample_n_frames_bucket_interval,
                                                                     args.video_sample_n_frames + sample_n_frames_bucket_interval,
                                                                     sample_n_frames_bucket_interval))]
                    ...
                    if args.train_mode != "normal":
                        ...
                        with torch.no_grad():
                            if config['transformer_additional_kwargs'].get('enable_clip_in_inpaint', True):
                                ...
                                if config['text_encoder_kwargs'].get('enable_multi_text_encoder', False):
                                    clip_attention_mask = torch.ones([bsc, unwrap_model(
                                        transformer3d).n_query]) if np.random.rand() < 0.50 else torch.zeros(
                                        [bsc, unwrap_model(transformer3d).n_query])
                                else:
                                    clip_attention_mask = torch.ones(
                                        [bsc, 8]) if np.random.rand() < 0.50 else torch.zeros([bsc, 8])
                        ...
                        with torch.no_grad():
                            prompt_embeds, prompt_attention_mask = \
                                encode_prompt(
                                    ...
                                    tokenizer_max_length=args.tokenizer_max_length, dtype=weight_dtype,
                                    enable_text_attention_mask=unwrap_model(transformer3d).config.get(
                                        "enable_text_attention_mask", True)
                                )
                            prompt_embeds_2, prompt_attention_mask_2 = \
                                encode_prompt(
                ...
                if args.noise_share_in_frames:
                    def generate_noise(bs, channel, length, height, width, ratio=0.5, generator=None, device="cuda",
                                       dtype=None):
                        noise = torch.randn(bs, channel, length, height, width, generator=generator, device=device,
                                            dtype=dtype)
                        for i in range(1, length):
                            noise[:, :, i, :, :] = ratio * noise[:, :, i - 1, :, :] + (1 - ratio) * noise[:, :, i, :, :]
                        return noise

                    noise = generate_noise(*latents.size(), ratio=args.noise_share_in_frames_ratio,
                                           device=latents.device, generator=torch_rng, dtype=weight_dtype)
                else:
                ...
                if args.loss_type != "sigma":
                    ...
                    # alpha_sqrt = alpha_sqrt.view(-1, 1, 1, 1, 1)  #

                    #pred_latents = (
                   #     noisy_latents * alpha_sqrt
                   #     - noise_pred * sigma_t * alpha_sqrt
                   # )
                    max_index = noise_scheduler.config.num_train_timesteps - 1  #
                    clamped_timesteps = timesteps.clamp(min=0, max=max_index)    #
                    sigma_t = noise_scheduler.sigmas[clamped_timesteps.cpu().long()].to(noisy_latents.device)

                    #sigma_t = noise_scheduler.sigmas[timesteps.cpu().long()].to(noisy_latents.device)
                    sigma_t = sigma_t.view(-1, 1, 1, 1, 1)  #
                    alpha_prod_t = 1 / (sigma_t ** 2 + 1).reshape(-1, 1, 1, 1, 1)  #
                    alpha_sqrt = alpha_prod_t ** 0.5
                    alpha_sqrt = alpha_sqrt.view(-1, 1, 1, 1, 1)
                    # pred_latents = noisy_latents * alpha_prod_t ** 0.5 - noise_pred * sigma_t * alpha_prod_t ** 0.5
                    pred_latents = (
                        noisy_latents * alpha_sqrt
                        - noise_pred * sigma_t * alpha_sqrt
                    )

                    pred_latents = pred_latents.to(dtype=next(vae.post_quant_conv.parameters()).dtype)
                    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                    vae = vae.to(device)  #

                    # target.shape: torch.Size([4, 16, 2, 6, 6])

                    pred_latents = pred_latents.to(device)

                    generated_frames = vae.decode(
                       pred_latents.to(dtype=next(vae.post_quant_conv.parameters()).dtype)
                      #force_not_quantize=True
                    ).sample
                    # todo:write loss here
                    #generated_frames: torch.Size([4, 3, 5, 48, 48])
                    # print("generated_frames:", generated_frames.shape)
                    pixel_values = batch['pixel_values'].cpu().mean(dim=2, keepdim=True)
                    # pixel_values = rearrange(pixel_values,
                    #                          'b t c (h_split h) (w_split w) -> b t (c h_split w_split) h w', h_split=2,
                    #                          w_split=2)
                    # print(generated_frames.permute(0,2,1,3,4).mean(dim=2, keepdim=True).shape)
                    # todo: fix with this same type problem
                    physics_module = physics_module.to(device)
                    physics_loss = physics_module(generated_frames.permute(0,2,1,3,4).mean(dim=2, keepdim=True).to(device))  # layer4: [B,T,C,H,W]
                    ...
                    if args.loss_type == "ddpm":
                        ...
                        # print("noise.shape:", noise_pred.shape) torch.Size([4, 16, 2, 6, 6])
                        weighting = compute_loss_weighting_for_sd3(weighting_scheme=args.weighting_scheme,
                                                                   sigmas=sigmas)
                        cst_loss = custom_mse_loss(noise_pred.float(), target.float(), weighting.float())
                        #todo :change loss weight here
                        alpha_x = 0.0001
                        print("cst_loss:", cst_loss)
                        print("phy_loss:", physics_loss)
                        loss = cst_loss + alpha_x * physics_loss
                        loss = loss.mean()
                ...
                if accelerator.sync_gradients:
                    if not args.use_deepspeed:
                        ...
                        max_grad_norm = linear_decay(args.max_grad_norm * args.initial_grad_norm_ratio,
                        ...
                        if trainable_params_total_norm / max_grad_norm > 5 and global_step > args.abnormal_norm_clip_start:
                            actual_max_grad_norm = max_grad_norm / min((trainable_params_total_norm / max_grad_norm),
                                                                       10)
                        else:
                        ...
                        if trainable_params_total_norm > 1 and global_step > args.abnormal_norm_clip_start:
                            for name, param in transformer3d.named_parameters():
                                if param.requires_grad:
                                    writer.add_scalar(f'gradients/before_clip_norm/{name}', param.grad.norm(),
                    ...
                    if not args.use_deepspeed and args.report_model_info and accelerator.is_main_process:
                        ...
                        writer.add_scalar(f'gradients/actual_max_grad_norm', actual_max_grad_norm,
                                          global_step=global_step)
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
