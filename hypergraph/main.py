...
def main():
    ...
    run_name = ''
    if run_name == '':
        load_model_step = 5000  # pretrained model was trained for 200k steps
    else:
    ...
    ip_kwargs = InitProcessGroupKwargs(timeout=datetime.timedelta(
        seconds=60 * 10))  # required to prevent timeout error when nonequal distribution of samplings to GPUs
    accelerator = Accelerator(mixed_precision='no', kwargs_handlers=[ddp_kwargs, ip_kwargs],
                              log_with='wandb' if wandb_username is not None else None)
    ...
    new_target_labels_dir = cur_dir
    # check if directory exists
    if os.path.exists(run_dir):
        if load_model_step is None:
            accelerator.print(
                'Directory already exists, please change run_name to train new model or provide load_model_step')
            return
    ...
    trainer.eval_target(new_target_labels_dir, guidance_scale=guidance_scale, num_preds=num_preds)
